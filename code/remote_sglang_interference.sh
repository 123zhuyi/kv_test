...
PORT="${PORT:-18000}"
RESULT_DIR="${RESULT_DIR:-$ROOT/results/interference_remote/sglang}"
...
echo "[interference] root=$ROOT"
date -u +"[interference] started_utc=%Y-%m-%dT%H:%M:%SZ"
nvidia-smi || true
...
python "$ROOT/run_interference_matrix.py" \
  --runtime sglang \
  ...
  --survivor-rate 5 \
  --blocker-gap-s 0.2 \
  --blocker-max-tokens 256 \
  --survivor-max-tokens 128 \
...
date -u +"[interference] finished_utc=%Y-%m-%dT%H:%M:%SZ"
echo "[interference] result_dir=$RESULT_DIR"
