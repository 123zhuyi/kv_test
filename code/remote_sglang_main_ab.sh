...
PORT="${PORT:-18000}"
RESULT_DIR="${RESULT_DIR:-$ROOT/results/main8b_remote/sglang}"
...
echo "[mainab] root=$ROOT"
date -u +"[mainab] started_utc=%Y-%m-%dT%H:%M:%SZ"
nvidia-smi || true
...
export PYTHONPATH="/root/autodl-tmp/sglang-main-pkg:${PYTHONPATH:-}"
...
python "$ROOT/instrument_sglang_main.py" "$PKG_ROOT" --diff-dir "$RESULT_DIR/provenance"
...
date -u +"[mainab] finished_utc=%Y-%m-%dT%H:%M:%SZ"
echo "[mainab] result_dir=$RESULT_DIR"
