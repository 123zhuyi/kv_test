...
PORT="${PORT:-18000}"
RESULT_DIR="${RESULT_DIR:-$ROOT/results/fixed8b_remote/sglang}"
...
echo "[fixedab] root=$ROOT"
date -u +"[fixedab] started_utc=%Y-%m-%dT%H:%M:%SZ"
nvidia-smi || true
...
python "$ROOT/instrument_sglang_fixed.py" "$PKG_ROOT" --diff-dir "$RESULT_DIR/provenance"
...
date -u +"[fixedab] finished_utc=%Y-%m-%dT%H:%M:%SZ"
echo "[fixedab] result_dir=$RESULT_DIR"
