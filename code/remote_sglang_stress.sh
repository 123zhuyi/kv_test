...
PORT="${PORT:-18000}"
RESULT_DIR="${RESULT_DIR:-$ROOT/results/stress_remote/sglang}"
...
echo "[stress] root=$ROOT"
date -u +"[stress] started_utc=%Y-%m-%dT%H:%M:%SZ"
nvidia-smi || true
...
python "$ROOT/run_stress_matrix.py" \
  --runtime sglang \
...
date -u +"[stress] finished_utc=%Y-%m-%dT%H:%M:%SZ"
